...
function addKeyAndValue(arr,key,value){
    ...
    arr.forEach((obj) => {
        tempObj = {};
        tempArr = Object.values(obj);
        tempObj[Object.keys(obj)] = tempArr[0];
        tempObj[key] = value;
...
function vowelCount(str){
   ...
   tempArr.forEach((letters) => {
    ...
    if (lowercase === 'a' || 
     ...
     if(tempObj[lowercase] === undefined) {
         tempObj[lowercase] = 1
     } else {
         tempObj[lowercase] = tempObj[lowercase] + 1
     }
...
function find(arr, searchValue) {
   ...
   if (result.toString().includes(searchValue)) {
       return result[0]
   } else {
       return undefined
   }
...
let noVowels = newArr.filter((arr) => {
   return arr !== 'a' &&
          arr !== 'e' &&
          arr !== 'i' &&
          arr !== 'o' &&
